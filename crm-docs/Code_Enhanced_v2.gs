// ============================================
// ATLAS CRM - Enhanced Code.gs v2.0
// WITH GDPR COMPLIANCE + QUOTA PROTECTION + SAFETY FEATURES
// ============================================
...
/**
 * Consolidate sheets with ENHANCED duplicate detection
...
function consolidateSheets(fileIds) {
  try {
    ...
    // Build lookup set for O(1) duplicate detection
    ...
    return {
      ...
      duplicateList: duplicateList.slice(0, 10), // Show first 10 duplicates
      errors: errors
    };
  } catch (error) {
...
/**
 * Check Gmail quota before creating draft
 */
...
function incrementGmailQuota() {
  ...
  const draftsToday = parseInt(cache.get(cacheKey) || '0');
  cache.put(cacheKey, String(draftsToday + 1), 86400); // 24 hour cache
  
  return draftsToday + 1;
}

/**
 * Save email as Gmail draft WITH quota protection
 */
...
/**
 * Batch create drafts with quota protection
 */
...
/**
 * Get or create a sheet (helper function)
 */
...
/**
 * Archive contacts with no activity in 2 years (GDPR compliance)
 * Run quarterly via time-based trigger
 */
function cleanupOldContacts() {
  try {
    ...
    // Add headers to archive sheet if empty
    ...
    // Process from bottom to top to avoid row index issues
    for (let i = data.length - 1; i >= 1; i--) {
      const row = data[i];
      const lastContact = row[dateCol];
      
      // Archive if no contact date or contact older than 2 years
...
/**
 * Delete a specific contact (GDPR right to deletion)
 */
